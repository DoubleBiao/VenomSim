...
class DroneSimEnv(gym.Env):
    def __init__(self):
        ...
        self.max_absolute_thrust = 2 * self.gravity
    ...
    def step(self, action):
        ...
        # test only
        #print('previous_distance: ', self.previous_distance)
        #print('self.position_hunter: ', self.position_hunter)

        # calculate reward and check if done
        
        reward = (self.previous_distance - self.distance) * 500 # even if chasing at max speed, the reward will be 10 / 120 * 500 = 41.67
        ...
        if self.distance > self.max_detect_distance or self.distance < self.min_detect_distance or self.iteration > self.max_iteration:
            ...
            self.episodes += 1
            self.iteration = 0
            if self.distance < self.min_detect_distance: reward = 100
    ...
    def get_state(self):
        ...
        # define state
        state = np.concatenate([np.array(orientation_hunter).flatten() / self.max_absolute_angle,
                                np.array(thrust_hunter).flatten() / self.max_absolute_thrust,
                                coordinate_state,
                                distance_state
                                ], 0)
    ...
    def reset(self):
        ...
        dronesim.siminit(np.squeeze(np.asarray(position_hunter)),np.squeeze(np.asarray(orientation_hunter)),\
                         ...
                         20,5)  #2 --> the speed upbound for hunter and target
